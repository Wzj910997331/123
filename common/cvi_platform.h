...
public:
    ...
    bool Trylock(){return (0x00 == pthread_mutex_trylock(&m_oMutex)) ? true : false;}
    int Timedlock(){
...
{
public:
    CVI_Scope_Mutex_SC(CVI_Mutex_SC& mutex):m_Mutex(mutex) 
    {
        m_Mutex.Lock();
    }

    virtual ~CVI_Scope_Mutex_SC()
    {
        m_Mutex.Unlock();
    }
...
public:
    ...
    CVI_Semaphore_SC(cvi_int32 u32InitNum = 0) {sem_init(&m_Sem, 0, u32InitNum);}
...
public:
    ...
    typedef enum
    {
        ...
        E_CVI_THREAD_PRIO_LOW     = 10,   // rtsp client thread default run at this priority.
        E_CVI_THREAD_PRIO_MEDIUM  = 0,    // GUI run at this priority.
        E_CVI_THREAD_PRIO_AUDIO   = -10,  // Foreground audio thread, aoThread and foreground adecThread.
        E_CVI_THREAD_PRIO_HIGH    = -15,  // Foreground rtsp client thread, receive audio data required by foreground audio thread.
    } CVI_THREAD_PRIO_E;

    static CVI_ERROR_CODE_E AdjustPrio(cvi_uint64 u64Pid, CVI_THREAD_PRIO_E ePrio);
...
public:
    ...
    CVI_Thread_SC(CVI_THREAD_ROUTINE_FN_T pFn, void *pParam, const char* pThreadName = "cvi_thread",\
...
{
public:
    CVI_TraceFunCall_C(const char* fileName, const char* funName):m_fileName(fileName),m_funName(funName)
    {
