...
public:
    ...
    CVI_ERROR_CODE_E TryPush(T &oMsg, cvi_uint32 u32Timeout); // u32Timeout = xx ms
...
template <class T>
CVI_ERROR_CODE_E CVI_Queue_SC<T>::Push(T &oMsg)
{
    pthread_mutex_lock(&m_oMutex);
...
template <class T>
CVI_ERROR_CODE_E CVI_Queue_SC<T>::Peek(T &oMsg)
{
    pthread_mutex_lock(&m_oMutex);
...
template <class T>
CVI_ERROR_CODE_E CVI_Queue_SC<T>::Pop(T &oMsg)
{
    pthread_mutex_lock(&m_oMutex);
...
template <class T>
CVI_ERROR_CODE_E CVI_Queue_SC<T>::Empty()
{
    pthread_mutex_lock(&m_oMutex);
