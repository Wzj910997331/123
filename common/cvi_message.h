...
public:
  ...
  template <typename T> static shared_ptr<T> MakeMsg() {
...
public:
  ...
  CVI_ERROR_CODE_E TryPush(shared_ptr<CVI_RespMsg_C> &pMsg,
                           cvi_uint32 u32Timeout) {
    if (0x55AA != pMsg->GetMagic()) {
      LOG(ERROR) << "plase use CVI_Msg_C::MakeMsg<T>(); to make msg.";
      CHECK_EQ(0, 1);
    }
...
public:
  // CVI_Queue_SC<CVI_Msg_C> *m_poRespQueue;// SYNC:RespQ; ASYNC(return
  // EVENT):EventQ; ASYNC(no return):NULL
  CVI_RespMsgQueue_C *m_poRespQueue;

  CVI_CmdMsg_C() { m_poRespQueue = NULL; }
  virtual void DUMP() {
    CVI_NVRLOGD("=========================================================");
    CVI_NVRLOGD("base DUMP=>");
    CVI_Msg_C::DUMP();
...
public:
  ...
  CVI_ERROR_CODE_E TryPush(shared_ptr<CVI_CmdMsg_C> &pMsg,
                           cvi_uint32 u32Timeout) {
    if (0x55AA != pMsg->GetMagic()) {
      LOG(ERROR) << "plase use CVI_Msg_C::MakeMsg<T>(); to make msg.";
      CHECK_EQ(0, 1);
