...
{
public:
    CVI_ERROR_CODE_E ReadPkt(CVI_BUFFER_QUEUE_ID queueID, CVI_RING_BUFFER_PKT_S &stRbPkt);
    CVI_ERROR_CODE_E WritePkt(CVI_RING_BUFFER_PKT_S &stRbPkt);

    CVI_ERROR_CODE_E SetPlayMode(CVI_BUFFER_QUEUE_ID queueID, CVI_BUFFER_PLAY_MODE eMode);

    cvi_uint32 PeekRemainSize(CVI_BUFFER_QUEUE_ID queueID);

    CVI_ERROR_CODE_E SetTimeBase(cvi_float ftb);
    CVI_ERROR_CODE_E UpdateReadPktSize(CVI_BUFFER_QUEUE_ID queueID, cvi_uint32 u32Diffms);
    CVI_ERROR_CODE_E UpdateReadIndexToKeyFrame(CVI_BUFFER_QUEUE_ID queueID);

    CVI_ERROR_CODE_E RegisterDestroyCallback(const CVI_RingBufferDestroyCallback_S &stCallback);
    CVI_ERROR_CODE_E UnregisterDestroyCallback(const CVI_RingBufferDestroyCallback_S &stCallback);
...
private:
    ...
    CVI_ERROR_CODE_E RemovePktReferPlayMode(const cvi_uint64 &u64CurrentPts, CVI_BUFFER_QUEUE_ID queueID);
...
public:
	static void Init();
	//static void Deinit();
	static CVI_ERROR_CODE_E AllocBuffer(cvi_uint32 u32Bytes,cvi_uint8* &pu8Buffer);
