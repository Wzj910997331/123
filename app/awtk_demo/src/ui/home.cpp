...
static ret_t onCanvasDrawRect(canvas_t *c, int drawX, int drawY, int drawW,
  ...
  if ((0 >= drawW) || (0 >= drawH)) {
    CVI_NVRLOGD("onCanvasDrawRect get failed r(%d, %d, %d, %d)", drawX, drawY,
                drawW, drawH);
    drawW = 500;
    drawY = 500;
    // return RET_FAIL;
...
static ret_t onCanvasPaint(void *ctx, event_t *e) {
  ...
  for (i = 0; i < CVIAPP_AIBOX_DEVICE_NUM_MAX; i++) {
    ...
    for (j = 0; j < pmsg->size; j++) {
      ...
      onCanvasDrawRect(c, drawRect.x, drawRect.y, drawRect.w, drawRect.h);
    }
...
static ret_t onCreate(void *ctx, event_t *e) {
  ...
  widget_t *canvas = canvas_widget_create(win, 0, 0, CVIAPP_IPC_VIEW_SCREEN_W,
