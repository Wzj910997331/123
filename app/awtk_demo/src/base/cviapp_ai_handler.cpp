...
static void __aiNotifyCallback(uint8_t u8Chn, CVIAPP_AiResult_S* pResult)
{
    ...
    case CVIAPP_AI_CVINVRTHREAD:
        {
            #if 1
            if((NULL != g_chnNotifyList[u8Chn])
                && (NULL != g_ipcViewPointer[u8Chn]))
            {
                ...
                for(i = 0; i < resultInfo->size; i++)
                {
                    ...
                    pmsg->pr[i].w = (uint32_t)(resultInfo->info[i].bbox.x2 - resultInfo->info[i].bbox.x1);
                    pmsg->pr[i].h = (uint32_t)(resultInfo->info[i].bbox.y2 - resultInfo->info[i].bbox.y1);
                }
                ...
                && (NULL != g_ipcViewPointer[u8Chn]))
            {
                ...
                for(i = 0; i < resultInfo->face.size; i++)
                {
                    ...
                    __scaleCviAiBbox2Rect(&resultInfo->face.info[i].bbox,
                                          resultInfo->face.width,
                                          resultInfo->face.height,
                                          CVIAPP_DEFAULT_GUI_WIDTH,
                                          CVIAPP_DEFAULT_GUI_HEIGHT,
                                          &pmsg->pr[i]);
                ...
                pmsg->sMatchResult
                    = (CVIAPP_MatchFace_S) resultInfo->sMatchResult;
                pmsg->u8Chn = u8Chn;
                pmsg->pIpcView = g_ipcViewPointer[u8Chn];
