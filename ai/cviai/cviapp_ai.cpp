...
static CVIAPP_AiContext_S __aiChnContextList[__MAX_AI_PROCESS][CVIAPP_AI_MODEL_MAX];
...
static void UpdateVpssCropGrpVdecParam(cvi_uint8 groupID, cvi_uint8 chnID, int w, int h);
...
// YUV -> JPEG
static void __cropJpegEnc(char *fileName, VIDEO_FRAME_INFO_S *pstVFrame ,int8_t chn)
{
    ...
    sprintf(jpgFileName,"%04d-%02d-%02d-%02d-%02d-%02d-%03d_%02d.jpeg",1900 + stm.tm_year, \
            1 + stm.tm_mon, stm.tm_mday, stm.tm_hour, stm.tm_min, stm.tm_sec, ms, chn);
    ...
    //CVI_NVRLOGW_CHECK((s32Ret != CVI_SUCCESS), "CVI_VENC_StartRecvFrame failed!");
    ...
    if(0x00 != stStat.u32CurPacks)
    {
        ...
        CVI_NVRLOGW_CHECK((s32Ret != CVI_SUCCESS), "CVI_VENC_ReleaseStream failed!");
    }
    else
    {
    ...
    //CVI_NVRLOGW_CHECK((s32Ret != CVI_SUCCESS), "CVI_VENC_StopRecvFrame failed!");

    fclose(fp);

}

static void initAiHandle_FaceTracking(cviai_handle_t *cviai_handle,int *aiHanleInitFlag,int i)
{
    ...
    s32Ret = CVI_AI_SetModelThreshold(*cviai_handle, CVI_AI_SUPPORTED_MODEL_RETINAFACE, 0.9);
    ...
    CVI_AI_SetModelThreshold(*cviai_handle, CVI_AI_SUPPORTED_MODEL_MOBILEDETV2_D0, OBJECT_DETECT_CONFIDENCE);
...
bool update_tracker(cviai_handle_t ai_handle, VIDEO_FRAME_INFO_S *frame,
                    ...
                    cvai_tracker_t *tracker_meta, int *miss_time, crop_face_info_t *cropFaceInfo, int8_t channel) {
      ...
      for (int j = 0; j < SAVE_TRACKER_NUM; j++) {
        if (fq_trackers[j].state == MISS) {
          ...
          if (face_meta->info[i].face_quality >= QUALITY_THRESHOLD) {
            ...
            CVI_S32 ret = CVI_AI_GetAlignedFace(ai_handle, frame, &fq_trackers[j].face, &face_meta->info[i]);
            if (ret != CVI_SUCCESS) {
...
//crop & encode jpeg
          ...
          pstCropInfo.stCropRect.u32Width = tracker_meta->info[i].bbox.x2 - tracker_meta->info[i].bbox.x1;
          pstCropInfo.stCropRect.u32Height = tracker_meta->info[i].bbox.y2 - tracker_meta->info[i].bbox.y1;
          ...
          UpdateVpssCropGrpVdecParam(groupID,0,pstCropInfo.stCropRect.u32Width,pstCropInfo.stCropRect.u32Height);
          ...
          CVI_NVRLOGW_CHECK((ret != CVI_SUCCESS), "CVI_VPSS_GetChnFrame failed!");


          //CropJpegEncInit(stCropFrameInfo.stVFrame.u32Width,  stCropFrameInfo.stVFrame.u32Height);
          __aiImageMutex.Lock();
          // YUV -> JPEG
          __cropJpegEnc(cropFaceInfo->cropJpegFileName,&stCropFrameInfo, channel);
          __aiImageMutex.Unlock();
          //CropJpegEncDeInit();

          ret = CVI_VPSS_ReleaseChnFrame(__VPSS_GRP_CROP, 0, &stCropFrameInfo);
          CVI_NVRLOGW_CHECK((ret != CVI_SUCCESS), "CVI_VPSS_ReleaseChnFrame failed!");
...
static void *__cviAiBoxAiThread(void *args)
{
   ...
   // CVIBSV_RTSP_CLIENT_STATUS_E eStatus = E_CVIBSV_RTSP_CLIENT_STATUS_IDLE;
        ...
        for (int i = 0; i < CVIAPP_AIBOX_DEVICE_NUM_MAX; i++)
        {
            ...
            //{
                if(NULL != pCtx->stParam.pGetFrameFun)
                {
                    ...
                    //CVI_NVRLOGD("__cviAiBoxAiThread try to get frame");
                    if(pCtx->stParam.pGetFrameFun(i, &stVideoFrameInfo[i]) == 0)
                    {
                    //CVI_NVRLOGD("__cviAiBoxAiThread try to get frame ret success(0)");
                    ...
                    //CVI_NVRLOGD("__cviAiBoxAiThread CVI_AI_RetinaFace success(%d), face.size=%d", s32Ret,face.size);
    ...
    //make sure release
    // CVI_NVRLOGD("\n#__cviAiBoxAiThread  make sure release....");
    // for (int i = 0; i < CVIAPP_AIBOX_DEVICE_NUM_MAX; i++)
    // {
    //     eStatus = CVIAPP_GetRtspStatus(i, E_CVI_DEV_PROFILE_MAIN);
...
static void *__aiObjectDetectThread(void *args)
{
    ...
    CVI_AI_SetModelPath(cviai_handle, CVI_AI_SUPPORTED_MODEL_MOBILEDETV2_PEDESTRIAN, PD_MODEL_PATH);
    CVI_AI_SetSkipVpssPreprocess(cviai_handle, CVI_AI_SUPPORTED_MODEL_MOBILEDETV2_PEDESTRIAN, false);
        ...
        for (int i = 0; i < CVIAPP_AIBOX_DEVICE_NUM_MAX; i++) {
            if (NULL != pCtx->stParam.pGetFrameFun) {
                if (pCtx->stParam.pGetFrameFun(i, &stVideoFrameInfo[i]) == 0) {
                    ...
                    CVI_NVRLOGD("__aiObjectDetectThread fps = %d", (int)((1 * 1000 * 1000 * 1000) / nsec));
                    clock_gettime(CLOCK_MONOTONIC, &tv1);
#endif
                }
            }
...
CVI_ERROR_CODE_E CVIAPP_AiStart(CVIAPP_AiParam_S &param)
{
    ...
    if(!pCtx_->bEnable)
    {
        ...
        pCtx_->pThread = new CVI_Thread_SC(pCtx_->pThreadFun, (void*) pCtx_, "aiThread",CVI_Thread_SC::E_CVI_THREAD_PRIO_LOW);
...
/******************************* __VPSS_GRP_AI ********************************/
    ...
    CVI_NVRLOGE_CHECK((s32Ret != CVI_SUCCESS), "CVI_VPSS_SetChnAttr:%d failed!", VpssChn);

    s32Ret = CVI_VPSS_EnableChn(VpssGrp, VpssChn);
    CVI_NVRLOGE_CHECK((s32Ret != CVI_SUCCESS), "CVI_VPSS_EnableChn:%d failed!", VpssChn);
...
/******************************* __VPSS_GRP_CROP ********************************/
    ...
    // CVI_NVRLOGE_CHECK((s32Ret != CVI_SUCCESS), "CVI_VPSS_SetChnAttr:%d failed!", VpssChn);
    // s32Ret = CVI_VPSS_SetChnAttr(VpssGrp, 2, &stVpssChnAttr);
    // CVI_NVRLOGE_CHECK((s32Ret != CVI_SUCCESS), "CVI_VPSS_SetChnAttr:%d failed!", VpssChn);
    ...
    // CVI_NVRLOGE_CHECK((s32Ret != CVI_SUCCESS), "CVI_VPSS_EnableChn:%d failed!", VpssChn);
    // s32Ret = CVI_VPSS_EnableChn(VpssGrp, 2);
    // CVI_NVRLOGE_CHECK((s32Ret != CVI_SUCCESS), "CVI_VPSS_EnableChn:%d failed!", VpssChn);
...
static CVI_ERROR_CODE_E CropJpegEncInit(int w,int h)
{
    ...
    stVencChnAttr.stVencAttr.bByFrame =  CVI_TRUE; /* get stream mode is slice mode or frame mode? */
